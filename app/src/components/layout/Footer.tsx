...
export function Footer() {
  return (
    <footer className="bg-ascend-black text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/kontakt" className="inline-flex items-center justify-center px-8 py-4 bg-ascend-orange text-white font-medium rounded-lg hover:bg-ascend-orange/90 transition-colors"><Mail className="w-5 h-5 mr-2" />Maila oss</Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8">
          ...
          <div>
            ...
            <ul className="space-y-3">{services.map((s) => (<li key={s.slug}><Link href={`/tjanster/${s.slug}`} className="text-sm text-white/60 hover:text-ascend-orange transition-colors">{s.title}</Link></li>))}</ul>
          </div>
          <div>
            <h4 className="font-semibold text-white mb-4 text-sm uppercase tracking-wider">Case</h4>
            <ul className="space-y-3">{caseStudies.slice(0, 4).map((c) => (<li key={c.slug}><Link href={`/case/${c.slug}`} className="text-sm text-white/60 hover:text-ascend-orange transition-colors">{c.title}</Link></li>))}</ul>
          </div>
          <div>
            <h4 className="font-semibold text-white mb-4 text-sm uppercase tracking-wider">Om Oss</h4>
            <ul className="space-y-3">{navItems.slice(2).map((item) => (<li key={item.href}><Link href={item.href} className="text-sm text-white/60 hover:text-ascend-orange transition-colors">{item.label}</Link></li>))}</ul>
          </div>
        </div>
      </div>
