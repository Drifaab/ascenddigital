    ...
    <section className="relative pt-32 pb-16 md:pt-40 md:pb-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
          <button onClick={handleBack} className="inline-flex items-center gap-2 text-sm text-ascend-gray-600 hover:text-ascend-orange transition-colors mb-8"><ArrowLeft className="w-4 h-4" />Tillbaka till case</button>
        </motion.div>
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <motion.div className="grid grid-cols-2 gap-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
            {caseStudy.stats.map((stat: { value: string; label: string; subtext?: string }, index: number) => (
