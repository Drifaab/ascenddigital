...
export function CaseCard({ caseStudy, index = 0 }: CaseCardProps) {
  const handleClick = () => {
    window.location.hash = `case/${caseStudy.slug}`
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ duration: 0.5, delay: index * 0.1 }}>
      <button onClick={handleClick} className="group block w-full text-left bg-white rounded-2xl border border-ascend-gray-100 overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
        ...
        <div className="p-5 md:p-6">
          ...
          <div className="flex flex-wrap gap-2 mt-4">{caseStudy.tags.slice(0, 3).map((tag: string, i: number) => (<Badge key={i} variant="default">{tag}</Badge>))}</div>
        </div>
      </button>
    </motion.div>
  )
